...
def count_tier_features(df, column=None):
    if column:
        return (
            df.assign(inverse_tier=lambda x: 3 - x.tier)
            ...
            .pivot(
                index=INDEX,
                columns=[column],
                values=["count", "null_tiers", "inverse_tier"],
            )
            .reset_index()
...
# %%
df_activity_hcp = (
    df_activity.merge(hcps.drop(columns=["region", "specialty"]), how="left", on="hcp")
    .rename(columns={"month": "activity_month"})
    .assign(
        activity_month_date=lambda x: x.activity_month.apply(
...
# %%
df_leaked_activity = df_full.assign(
    month_date=lambda x: x.month.apply(lambda x: pd.to_datetime(x, format="%Y-%m"))
...
df_lag_3m = df_full_activity[
    df_full_activity.activity_month_date + pd.DateOffset(months=3)
    >= df_full_activity.month_date
]
...
basic_features_specialty_3m = rename_cols_3m(
    count_tier_features(df_lag_3m, "specialty")
)

# %%
df_activity_features = (
    df_full.merge(basic_features, on=INDEX, how="left")
