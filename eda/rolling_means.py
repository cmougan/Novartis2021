...
(
    subset_df.set_index("date").assign(
        rolling_births=lambda d: d["births"]
        .shift(1)
...
def calc_rolling_mean(dataf, column=None, setting="30D"):
    return dataf.groupby("state")[column].transform(
        lambda d: d.shift(1).rolling(setting, min_periods=1).mean()
    )


# %% Create a df with all data, no filter
unfilter_df = df[["state", "date", "births"]].assign(
    date=lambda d: pd.to_datetime(d["date"], format="%Y-%m-%d")
)

# %% This does the whole process of adding the rolling function
(
    unfilter_df.set_index("date")
...
(
    unfilter_df.set_index("date")
    .assign(
        rolling_births=lambda d: rolling_fn(
            d,
            column="births",
            groupby_cols="state",
            function="mean",
