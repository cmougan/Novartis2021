...
def base_metric(df, market_size):
    ...
    df_country = (
        df.groupby(["month", "brand"])[["predictions", "actuals"]].sum().reset_index()
    )
    ...
    MAE = (metric_country / market_size.sum()) + (metric_region / market_size).groupby(
        ["brand"]
    ).mean()
...
def ComputeMetrics(
    solution: pd.DataFrame, sales_train: pd.DataFrame, ground_truth: pd.DataFrame
):
    ...
    if df["predictions"].isna().sum() > 0:
        raise Exception(
            "Submitted solution is missing some of the required brand - region - month combinations"
        )
    ...
    market_size = (
        sales_train.query('brand == "brand_12_market"')
        .groupby("region")
        .mean()
        .squeeze()
    )
